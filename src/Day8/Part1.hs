...
import GHC.Tc.Gen.Head (isVisibleArg)
...
getVisibles field =
  let width = length (head field)
      height = length (traceShow field field)
...
isVisible i j height width field =
  let value = at i j field
      ...
      rights = map (\v -> at i v field) (take (width - j - 1) [(i - 1)..])
...
isTallest value others =
    not $ any (>= value) (trace ((show value) ++ show others) others)
